...
// Subscribe to services collection with real-time updates
export const subscribeToServices = (callback: (services: Service[]) => void): (() => void) => {
  ...
  if (activeListeners['services']) {
    return activeListeners['services'];
  }

  const servicesCollection = collection(db, 'services');
  const unsubscribe = onSnapshot(servicesCollection, (snapshot) => {
    const services = snapshot.docs.map(convertService);
    callback(services);
  }, (error) => {
  ...
  activeListeners['services'] = unsubscribe;
  return unsubscribe;
};

// Subscribe to a single service with real-time updates
export const subscribeToService = (id: string, callback: (service: Service | null) => void): (() => void) => {
  // Check if we're using demo configuration
  if (isDemoMode) {
    // Simulate real-time updates with mock data
  ...
  // Check if we already have an active listener for this service
  const listenerKey = `service_${id}`;
  ...
  const unsubscribe = onSnapshot(serviceRef, (doc) => {
    if (doc.exists()) {
      callback(convertService(doc));
    } else {
      callback(null);
    }
  }, (error) => {
    console.error(`Error subscribing to service ${id}:`, error);
    callback(null);
  });
...
// Get available services (non-realtime)
...
// Get featured services (non-realtime)
...
// Get service by ID (non-realtime)
